...
class Solution:
    def minFlips(self, s: str) -> int:
        ...
        for i in range(n):
            pre[i][0] = (0 if i == 0 else pre[i - 1][1]) + eq(s[i], 1)
            pre[i][1] = (0 if i == 0 else pre[i - 1][0]) + eq(s[i], 0)
        ...
        if n % 2 == 1:
            ...
            for i in range(n - 1, -1, -1):
                suf[i][0] = (0 if i == n - 1 else suf[i + 1][1]) + eq(s[i], 1)
                suf[i][1] = (0 if i == n - 1 else suf[i + 1][0]) + eq(s[i], 0)

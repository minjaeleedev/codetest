...
class SegmentTree:
    ...
    def add(self, left: int, right: int, val: int):
        tag = LazyTag()
        tag.to_add = val
        self._update(left, right, tag, 1, self.n, 1)
    ...
    def _build(self, data, left: int, right: int, i: int):
        if left == right:
        ...
        self._build(data, mid + 1, right, (i << 1) | 1)
        self._pushup(i)

    def _update(self, target_l: int, target_r: int, tag, left: int, right: int, i: int):
        if target_l <= left and right <= target_r:
        ...
        if target_r > mid:
            self._update(target_l, target_r, tag, mid + 1, right, (i << 1) | 1)
        self._pushup(i)

    def _find(self, target_l, target_r, val, left: int, right: int, i: int):
        ...
        if target_r >= mid + 1:
            res = self._find(target_l, target_r, val, mid + 1, right, (i << 1) | 1)
            if res != -1:
                return res
